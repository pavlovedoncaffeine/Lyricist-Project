...
def getTracksFromPlaylist(spLyr=None, plistID=None):
    ...
    while index < total:
        queryResult = spLyr.playlist_tracks(
            plistID, fields='items(track(name, artists(name),id,explicit,duration_ms))', offset=index)['items']
        index += 100
...
# To-do: implement a rudimentary system to pull lyrics and write to file in the Data Folder.s
    ...
    with writeLog(open(LOGFILE, 'a')):
        ...
        if spLyricist is not None:
            ...
            for track in playlistTracks:
                ...
                # (4)
                result = saveLyrics(track['track_name'], track['artists'])
